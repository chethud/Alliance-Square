          ...
          <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
            {blogPosts.map((post, index) => (
              <FadeIn key={post.id} delay={index * 0.05}>
                <Link href={`/insights/${post.slug}`} className="premium-card group block">
                  ...
                  <div className="p-6 md:p-7">
                    ...
                    <h2 className="mt-4 text-xl font-bold text-charcoal transition-colors group-hover:text-brand-cyan md:text-2xl">
                      {post.title}
                    </h2>
                    <p className="mt-3 line-clamp-3 text-sm text-cool-gray">{post.excerpt}</p>
